 function editContent_EN()
 {
    $(".edit-content-en").on("click",function(e){
        ...
        // read article 
        $.post(current_url + 'content-edit-en', {hash : hash})
        .done(function(read){
            ...
            <input type="hidden" name="status" value="${!read['response']['status'] ? 'Draft' : read['response']['status']}"/>
            ...
            <div class="col-auto">
            ${(!read['response']['title']) ? `Membuat Konten EN <b>${read['response']['title']}</b>` : `Edit Konten EN <b>${read['response']['title']}</b> `}
            </div>
            ...
            <input value="Draft" class="form-check-input" type="radio" name="status_h" id="status-draft" ${(!read['response']['status'] || read['response']['status'] == 'Draft') ? 'checked' : ''}>
...
function formeditContent_EN(dialog)
{
    ...
    form.on("submit",function(e){   
        ...
        $.post(current_url + 'content-update-en', form.serialize() , {}, 'json')
        .done(function(data){
